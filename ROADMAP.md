...
- [x] Delete dead en keys: `tool.mood.availableIn`, `tool.mood.switchToEnglish`,
      `tool.mood.inThePast7Days`, `tool.i18n.showMarkup`, `tool.i18n.keepTags`
      (the last two went with the `isBody` branch)
...
- [ ] Small stuff: ~~toast timer race~~ (done); per-tool history date formats; sticky
...
## Done since the audits

Not from the three lists above, so recorded here rather than checked off.

- [x] **Translation staleness.** Every non-English locale records a fingerprint
      of the English each string was translated from, so an edited English
      sentence shows reviewed English in the other languages instead of a
      translation of a sentence that no longer exists. `translation/hash.js`,
      `stamp.js`; `node translation/stamp.js report` is the retranslation
      worklist.
- [x] **Translation pipeline repair.** `translation/source/*.json` was a
      hand-kept snapshot that had fallen back to the pre-split card shape;
      merging from it would have blanked every sentence in es, fr and ko.
      Generated by `snapshot.js` now.
- [x] **Local English editor** (`editor/`, 127.0.0.1:3906, desktop shortcut).
      Edit any sentence in the page, save to `locale.en.js`, publish to obiana.
- [x] **Real icons** on all 35 guide cards, the six home tiles and the bottom
      nav, from Tabler (MIT). The FAQ icon is deliberately untouched.
- [x] **Obiana in the header** in all 17 languages; hero text translated into
      the nine locales that lacked it. Zomi still falls back to English and
      needs a native speaker.
- [x] Section hero subtitles removed; the home one kept.
- [x] Reassurance and padding clauses cut across the whole guide.

### Known open

- Zomi: the UI block, the hero, and the language-picker prompt are all English
  or unconfirmed. Needs a speaker, not another machine pass.
- One machine-translation run covers most of what is left in section 1 and the
  staleness backlog. Needs PPQ credits.
